...
object METransposer {
  ...
  abstract class Common extends Transposer.Common {
    ...
    private def convertItem(stack: IAEItemStack): util.Map[String, AnyRef] = {
      Registry.convert(Array[AnyRef](stack.getItemStack)).collect { case hash: util.HashMap[_, _] => NetworkControl.hashConvert(hash) }.headOption match {
        case Some(hash) =>
          hash.put("size", Long.box(stack.getStackSize))
          hash.put("isCraftable", Boolean.box(stack.isCraftable))
          hash
        case _ => new util.HashMap[String, AnyRef]()
      }
    }

    private def convertFluid(stack: IAEFluidStack): util.Map[String, AnyRef] = {
      Registry.convert(Array[AnyRef](stack.getFluidStack)).collect { case hash: util.HashMap[_, _] => NetworkControl.hashConvert(hash) }.headOption match {
        case Some(hash) =>
          hash.put("amount", Long.box(stack.getStackSize))
          hash.put("isCraftable", Boolean.box(stack.isCraftable))
          hash
        case _ => new util.HashMap[String, AnyRef]()
      }
    ...
    def getItemsInNetwork(context: Context, args: Arguments): Array[AnyRef] = {
      ...
      result(p.getStorage.getItemInventory.getStorageList.view.map(convertItem).filter(matches(_, filter)).toArray)
    }
    ...
    def getFluidsInNetwork(context: Context, args: Arguments): Array[AnyRef] = {
      ...
      if (!p.isActive) return result(Unit, "no ME network")
      val filter = networkFilter(args)
      result(p.getStorage.getFluidInventory.getStorageList.view.map(convertFluid).filter(matches(_, filter)).toArray)
    }
    ...
    def getItemsInNetworkById(context: Context, args: Arguments): Array[AnyRef] = {
      ...
      result(p.getStorage.getItemInventory.getStorageList.view.filter(item => itemFilterSet.contains(item.getItem)).map(convertItem).toArray)
    }
