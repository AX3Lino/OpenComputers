...
trait InventoryTransfer extends traits.WorldAware with traits.SideRestricted {
  ...
  protected def virtualSideSwapError: String = "cannot swap with a virtual side"
  ...
  def swap(context: Context, args: Arguments): Array[AnyRef] = {
    if (isVirtualSide(args, 0) || isVirtualSide(args, 1)) return result(Unit, virtualSideSwapError)
