...
// Adapter's neighbor-block driver scanning, factored out so it can also be fused into the ME Actuator family.
trait AdapterInterfacing extends Environment with OpenSides with Analyzable {
